...
const ToastProvider = ToastPrimitives.Provider as React.ComponentType<
...
const toastVariants = cva(
    "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg transition-all will-change-[transform,opacity] data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-[toast-slide-in-right_200ms_ease-out_forwards] data-[state=closed]:animate-[toast-slide-out-right_200ms_ease-in_forwards] data-[swipe=end]:animate-[toast-slide-out-right_200ms_ease-in_forwards]",
    {
        variants: {
            variant: {
                default: "border bg-white text-[#222]",
                destructive:
...
>(({ className, ...props }, ref) => (
    <ToastPrimitives.Close
        ref={ref}
        className={cn(
            "text-[#222]/50 hover:text-[#222] absolute top-2 right-2 rounded-md p-1 opacity-0 transition-opacity group-hover:opacity-100 group-[.destructive]:text-red-300 hover:group-[.destructive]:text-red-50 focus:opacity-100 focus:ring-2 focus:outline-hidden focus:group-[.destructive]:ring-red-400 focus:group-[.destructive]:ring-offset-red-600",
            className
        )}
        toast-close=""
        {...props}
