...
class Animation {
    ...
    static layout = {
        header: (target) => {
            ...
            ScrollTrigger.create({
                ...
                onEnter: () => {
                    gsap.to("#header a, #header h5, #progress p", {
                        color: "#000000",
                    });
                    gsap.to("#progress .scroll_background", {
    ...
    static section01 = {
        ...
        scroll: (target) => {
            ...
            const titlePositionBottom = gsap.utils.toArray(
                "#intro .title .bottom"
            );
            ...
            // 글자 분리
            for (let i = 0; i <= titlePositionTop.length - 1; i++) {
                splitText(titlePositionTop[i]);
            }
            for (let i = 0; i <= titlePositionBottom.length - 1; i++) {
                splitText(titlePositionBottom[i]);
            }
    ...
    static section04(target, isMobile) {
        ...
        if (target.container.current) {
            const scrollTween = gsap.to(sections, {
                xPercent:
                    -100 * (isMobile ? sections.length : sections.length - 1),
                ease: "none",
                scrollTrigger: {
